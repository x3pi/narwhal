...
for i in $(seq 0 $((NODES-1))); do
    ...
    tmux new -d -s "primary-$i" "RUST_LOG=info,primary=info,network=info  $primary_cmd > '$primary_log_file' 2>&1"
    ...
    for j in $(seq 0 $((WORKERS_PER_NODE-1))); do
        ...
        tmux new -d -s "worker-$i-$j" "RUST_LOG=info,primary=info,network=info  $worker_cmd > '$worker_log_file' 2>&1"
    done
done
